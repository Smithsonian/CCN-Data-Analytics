...
cores <- read_csv("data/report_data/CONUS_cores.csv", guess_max = 7000)
wetland_area <- read_csv("data/report_data/state_wetland_area_ccap.csv")
...
# spatial_rep_metric
spatial_rep_metric <- read_csv("data/report_data/spatial_rep_by_state.csv") %>% 
  arrange(-spatial_representativeness_metric) %>% 
  mutate(state = state.abb[match(state,state.name)])

spatial_rep_metric <- bind_rows(spatial_rep_metric, data.frame(state = wetland_area$state[! wetland_area$state %in% spatial_rep_metric$state],
                                                       stringsAsFactors = F))

spatial_rep_metric$spatial_representativeness_metric[spatial_rep_metric$state == "PA"] <- NA

# habitat_rep_metric
habitat_rep_metric <- read_csv("data/report_data/habitat_representativeness_metric.csv") %>% 
  arrange(-euclidean_distance)

habitat_rep_metric <- bind_rows(habitat_rep_metric, data.frame(state = wetland_area$state[! wetland_area$state %in% habitat_rep_metric$state],
                                                               stringsAsFactors = F))
...
# Join all state metrics/scores into a single data frame
all_states_quadrats <- quantity_metric %>% 
  full_join(quality_metric) %>% 
  full_join(spatial_rep_metric) %>% 
  full_join(habitat_rep_metric)

write_csv(all_states_quadrats, "data/report_data/state_metrics.csv")
...
quantity_plot <- ggplot(data = all_states_quadrats, aes(x = reorder(state, quantity_metric2), y = quantity_metric, color = detailed_report)) +
  geom_point(pch=16) +
...
quality_plot <- ggplot(data = all_states_quadrats, aes(x = reorder(state, quality_metric2), y = quality_metric, color = detailed_report)) +
  geom_point(pch=16) +
  geom_segment(aes(xend=state, yend=0)) +
  xlab(NULL) +
...
spatial_plot <- ggplot(data = all_states_quadrats, aes(x = reorder(state, spatial_representativeness_metric2), y = spatial_representativeness_metric, color = detailed_report)) +
  geom_point(pch=16) +
  geom_segment(aes(xend=state, yend=0)) +
  xlab(NULL) +
...
habitat_plot <- ggplot(data = all_states_quadrats, aes(x = reorder(state, euclidean_distance2), y = euclidean_distance, color = detailed_report)) +
  geom_point(pch=16) +
  geom_segment(aes(xend=state, yend=0)) +
  xlab(NULL) +
...
grid.arrange(arrangeGrob(nrow=2, ncol=2,
...
ggsave("figures/All 4 Quadrat figures.jpg", dpi=300, width = 6.5, height = 6.5, grobs)
ggsave("figures/All 4 Quadrat figures.pdf", width = 6.5, height = 6.5, grobs)
