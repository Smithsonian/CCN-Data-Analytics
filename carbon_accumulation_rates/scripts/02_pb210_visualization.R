...
unique(c(depths$pb210_unit, depths$ra226_unit, depths$pb214_unit, depths$bi214_unit))
...
convertToBqPerKg <- function(value, unit) {
  ...
  output_value <- ifelse(! is.na(unit),
                         ...
                         ifelse(unit %in% c("disintegrationsPerMinutePerGram"),
                                ...
                                ifelse(unit %in% c("becquerelsPerGram"),
                                       value * 1000,
                                       ifelse(unit %in% c("picocuriesPerGram"),
                                              # * 0.037 Bq / 1 pC * 1000g / 1 kg
                                              value * 0.037 * 1000, 
                                              value
                                            )))),
                         NA)
...
unique(pb_depths$study_id[pb_depths$pb210_unit %in% c("disintegrationPerMinutePerGram", 
                                                      "disintegrationsPerminutePerGram",
...
plot(log(pb_depths_xspb210_consistent$total_pb210_activity_se), log(pb_depths_xspb210_consistent$total_pb210_activity))
...
for (i in 1:nrow(study_site_combos)) {
  ...
  output_name <- paste0("carbon_accumulation_rates/output/figs/pb210/", study_site_combos$study_id[i], "_", study_site_combos$site_id[i], "_210Pb.jpg")
  ggsave(output_name, output_file, width = width, height = height, units = "in")
}

write_csv(pb210_vis, "carbon_accumulation_rates/output/tabs/pb210_profiles.csv")
