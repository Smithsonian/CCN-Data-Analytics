# Meng synthesis with geography

mengdat <- read_csv("coastal_wetland_nggi/data/original/US-BC-Analysis-1-105 JH edit.csv")
...
cores <- mengdat %>% # read straight from synthesis
  rename(longitude=Longitude, latitude=Latitude) %>% 
  mutate(across(c("latitude", "longitude"), as.numeric)) %>% 
  mutate(core_id = as.character(1:n()))
...
core_geography %>%
  filter(!is.na(delSOC1Pb)|!is.na(delSOC2Cs)) %>% 
  leaflet() %>%
  addProviderTiles(providers$CartoDB) %>%
  addCircleMarkers(lng = ~longitude, lat = ~latitude,
                   radius = 2, label = ~First_Author)
...
write_csv(core_geography, "coastal_wetland_nggi/data/original/US-BC-Analysis-1-105_w_geog.csv") # core table is now passed onto the next post-processing function
