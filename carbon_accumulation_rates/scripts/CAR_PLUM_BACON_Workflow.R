...
for (i in 1:nrow(cores)) {
  ...
  if (nrow(pb_subset)>=1) {
    ...
    # Check to see if the file is already done
    if (file.exists(paste("BACON/Cores/", temp_core_name, "/", temp_core_name, ".csv", sep="")) &
        overwrite_stuff == F) {
      print("... already done.")
    } else {
      # If not write Bacon file
      # Create file structure
      if (! file.exists(paste("BACON/Cores/", temp_core_name, sep=""))) {
        dir.create(paste("BACON/Cores/", temp_core_name, sep=""), recursive = T)
      }
      ...
      surface_df <- data.frame(labID = "surface",
                               age = 1950 - surface_date,
                               depth = 0,
                               error = 1, 
                               cc = 0)
      
      bacon_ages <- bind_rows(surface_df, ancillary_subset) %>% select(-study_id, -site_id, -core_id) %>% 
        arrange(depth) %>% 
        select(labID, age, error, depth, cc)
      
      write_csv(bacon_ages, paste("BACON/Cores/", temp_core_name, "/", temp_core_name, ".csv", sep=""))
      ...
      bd_loi <- depths %>% 
        filter(study_id == this_study_id,
               site_id == this_site_id,
               core_id == this_core_id) %>% 
        select(depth_min, depth_max, fraction_organic_matter, dry_bulk_density, fraction_carbon) %>% 
        filter(!is.na(fraction_organic_matter)|!is.na(dry_bulk_density)|!is.na(fraction_carbon))
      
      temp_interval <- min(bd_loi$depth_max - bd_loi$depth_min)
      
      try({
        
        
        
        rbacon::Bacon(core = temp_core_name,
                      coredir="BACON/Cores/",
                      suggest=F, ask=F, remember=F,
                      verbose=F)
        ...
        friendlyBaconNames <- c("logOdds",
                                paste("accRate[", (1:n_depth_ints)*temp_interval,"]", sep=""),
                                "memory")
        names(friendlyBacon) <- friendlyBaconNames
        ...
        write_csv(tidyBacon, paste("BACON/Cores/", temp_core_name, "/", temp_core_name, "_tidy_plum.csv", sep=""))
        
      }) 
    }
  } else {
    # Else do nothing in this work flow
    paste("... Dated Horizons need to be run separately.")
  }
}
