...
cs_subset <- cs_peaks %>%
  select(study_id, site_id, core_id, habitat, year, min_possible_peak_depth, depth_med, max_possible_peak_depth, sig_level, high_accretion, low_accretion) %>% 
...
historical_horizon_subset <- historical_horizons %>% 
  select(study_id, site_id, core_id, habitat, year, depth_min, depth_max, high_accretion, low_accretion) %>% 
  group_by(study_id, site_id, core_id) %>% 
...
all_together_now <- bind_rows(cs_subset, historical_horizon_subset) %>% 
  ...
  select(study_id, site_id, core_id, habitat, year, data_type, depth_min, 
...
avg_carbon_per_horizon <- all_together_now_gapfilled %>% 
  ...
  group_by(study_id, site_id, core_id, horizon_id, data_type, high_accretion, low_accretion) %>% 
  filter(depth_max <= horizon_depth_med) %>% 
  summarise(carbon_density = mean(oc_gap_filled*dry_bulk_density_gap_filled)) %>% 
  ungroup() %>% 
